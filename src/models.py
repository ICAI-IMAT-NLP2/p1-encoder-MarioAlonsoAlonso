...
class AttentionHead(nn.Module):
    ...
    def __init__(self, d_model: int, d_k: int, d_q: int, d_v: int):
        ...
        self.wq = torch.nn.Linear(d_model, d_q)
    ...
    def scaled_dot_product_attention(self, q, k, v):
        ...
        scores = torch.matmul(q, torch.transpose(k, 1, 2)) / math.sqrt(dim_k)

        # Apply the softmax function to obtain the attention weights.
        weights = torch.nn.functional.softmax(scores, 2)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([AttentionHead(d_model, d_k, d_q, d_v)
    ...
    def forward(self, hidden_state):
        ...
        if not (self.d_model % self.num_attention_heads == 0):
            raise RuntimeError("The model dimension must be divisible by the number of attention heads")
        
        head_outputs = [head(hidden_state) for head in self.heads]

        concat = torch.cat(head_outputs, dim=-1) #(batch, seq_len, num_heads*d_v)

        x = self.output_linear(concat) #(batch, seq_len, d_model)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feedforward_output = self.feed_forward(hidden_state)
